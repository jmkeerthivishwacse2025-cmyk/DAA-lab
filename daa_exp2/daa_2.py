...
def complexity_comparison():
    ...
    print(f"{'Algorithm':<15}{'Best Time':<15}{'Average Time':<18}{'Worst Time':<18}{'Space'}")
    print("-" * 75)

    print(f"{'Naive':<15}{'O(n)':<15}{'O(n*m)':<18}{'O(n*m)':<18}{'O(1)'}")
    print(f"{'KMP':<15}{'O(n+m)':<15}{'O(n+m)':<18}{'O(n+m)':<18}{'O(m)'}")
    print(f"{'Rabin-Karp':<15}{'O(n+m)':<15}{'O(n+m)':<18}{'O(n*m)':<18}{'O(1)'}")

    print("\nSimple Comparison:")
    print("Naive Search  : Easy to implement but slower for large text.")
    print("KMP Algorithm : Faster because it avoids repeated comparisons.")
    print("Rabin-Karp    : Uses hashing, fast on average, slow in worst case.")
